...
class Administrations (BaseModel):
...
class Normes (BaseModel):
    DesignationFr = models.CharField(max_length=50)
    AbreviationFr = models.CharField(max_length=20, null=True)

    def __str__(self):
        return f"Norme : {self.AbreviationFr} "
...
class Localisation (BaseModel):
    ...
    def __str__(self):
        return f" Localisation : {self.region}_{self.departement}_{self.arrondissement}_{self.pays} quartier : {self.Quartier} "

class Immeubles (BaseModel):
    ...
    def __str__(self):
        return f" immeuble : {self.Designation} "

class Loges (BaseModel):
    Designation = models.CharField(max_length=50)
...
class Dossiers_Reglements (BaseModel):
