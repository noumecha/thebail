$(function () {
  ...
  setVisible(
    '#id_immeubles-0-Type_localisation',
    '#id_immeubles-0-Ville, #id_immeubles-0-pays, #id_immeubles-0-Rue',
    '1'
  ); // 1 - Extérieure
  setVisible(
    '#id_immeubles-0-Type_localisation',
    '#id_immeubles-0-region,  #id_immeubles-0-departement, #id_immeubles-0-arrondissement, #id_immeubles-0-Quartier',
    '2'
  ); // 2 - National
  ...
  setVisible(
    '#id_Type_personne',
    '#id_Nom_prenom, #id_Maticule, #id_Type_id_bailleur, #id_Num_doc, #id_Date_delivrance_doc, #id_Document_identification',
    '2'
  ); // 2 - personne physique
  ...
  function collectAyantsDroits() {
    ...
    $('#ayantdroit-collecte-table tbody tr[data="ayantdroit"]').each(function () {
      ...
      const obj = {
        ...
        Date_prise_effet_certificat_non_effet: $row
          .find('input[name="Date_prise_effet_certificat_non_effet_hidden[]"]')
          .val()
      };
  ...
  function getNonMandatementData() {
    ...
    $("#nonmandatement-collecte-table tbody tr[data='nonmandatement']").each(function () {
      ...
      let dateSignature = row.find("input[name='Date_signature_hidden[]']").val();
  ...
  function collectImmeubles() {
    ...
    $('#immeubles-0').each(function (idx) {
      ...
      let elements = [];
      $(this)
        .find("[name^='immeubles-0-element_']")
        .each(function () {
          ...
          if (name.endsWith('_statut')) {
            ...
            if (statut && nombre <= 0) {
              errors.push(
                `Dans l'immeuble n°${idx + 1}, l’élément #${id} est coché mais aucun nombre valide n’a été saisi.`
              );
            }
  ...
  $(document).on('click', '#submit-id-save', function (e) {
    ...
    if (window.nonMandatementFiles) {
      // Ajouter les fichiers liés
